...
export const countNonEmpty = function (object) {
  let c = 0;
...
export const repeaterData = (value, returnArray = false, removeEmpty = true) => {
  ...
  ({
    ...
    return el("div", { className: "repeater-row-wrapper" }, [
      ...
      el(
        ...
        addOnNonEmpty && index === parentValue.length - 1
          ? null
          : el(
              c.Button,
              {
...
c.RepeaterControl = wp.compose.withInstanceId(function (_ref) {
  ...
  value.map((v) => {
    if (typeof v._key === "undefined") v._key = key++;
    else {
      key = v._key;
    }
  });
  ...
  return el(
    ...
    [
      el(SortableList, {
        ...
        helperContainer: function () {
          ...
          return typeof this.container !== "undefined"
            ? this.container
            : jQuery(".edit-post-sidebar").get(0);
        },
      ...
      !addOnNonEmpty && (!max || value.length < max)
        ? el(
            c.Button,
            {
...
export const RepeaterControl = c.RepeaterControl;
